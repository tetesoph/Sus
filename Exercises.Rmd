...
library(haven)
```
### AN: Changed .sav file to.csv file; still uaing subset data
...
### Replicate this PATH for all tasks
...
summary(reg_linear)
```
table(Allbus_subset$party)
```{block, box.title = "2", box.body = list(fill = "white"), box.icon = "fa-star"}
As the next step in our analyses, we want to run a logistic regression model with the variable indicating whether people sre in favour eo immigration due to *economic reasons* and trust in newspaper, trust in the government, and choice of party as predictors.
```
...
```{r first-exercise, solution = TRUE}
reg_model <- lm(sum_acceptance ~ risk_self + risk_surroundings + sex*trust_government + eastwest,
                data = corona_survey)
summary(reg_model)
```
