...
```{r}
library(tidyverse, readr, readxl, remotes, dplyr, janitor, correlation, haven, purrr, psych, summarytools, sjlabelled, janitor, correlation, effectsize, tidyr, ggplot2, easypackages, tibble, parameters, see, performance, sjPlot, report, broom, stargazer)
```
...
### ANNOTATION:*six* distinct strings right?
...
### I think it would be nice to display the different outcomes of 1. setup names and 2. changed variable names in 'cool_data_frame'
...
### AN: I called the variables 'first' and 'second' as stated in the description.
```{r sixth.exercise, solution = TRUE}
...
library(haven)
Allbus <- read_dta("ZA5280_v1-0-0.dta")
```
...
write_stata(Allbus, "Allbus_final_final.dta")
```
...
## Idea
Fortunately, our lovely intern ;) found an English version of the data set, so we do not have to translate each variable from German to English. Nevertheless, this is an important skill and you will probably need it in the future. As in the presentation, we will use data from the [German General Social Survey - ALLBUS 2021] (*AKTUELLER LINK* https://www.gesis.org/gesis-panel/coronavirus-outbreak/public-use-file-puf) for this exercise. You should (have) download(ed) the dataset in `.dta` format and saved it in a folder caller `data` within the folder containing the materials for this workshop. Also remember that it is helpful to consult the [codebook for the data set](*AKTUELLER LINK*
https://dbk.gesis.org/dbksearch/download.asp?id=67378).
...
library(tidyverse)
Allbus <- read_dta("Allbus_final_final.dta")
```
...
## Idea: Change to pt01-pt20 (problem: gaqps in consecutive order e.g. pt05) - Trust variables in German institutions
```{block, opts.label = "clues"}
...
```{r read-data, message = F}
library(tidyverse)
gp_covid <- read_csv2("./data/XXXXX.csv")
```
...
```{block, box.title = "1", box.body = list(fill = "white"), box.icon = "fa-star"}
As a first exercise, using `base R`, let's create a new data set named `Allbus_single_` that only contains data from respondents who reported being single.
```
...
```{block, box.title = "2", box.body = list(fill = "white"), box.icon = "fa-star"}
Now, let's use the `dplyr` function for filtering rows: Create an object named `Allbus_male_sober_driver` that only contains respondents who report that they are male as well as never having driven a car while being drunk in the past.
```

```{block, opts.label = "clues"}
The names of the variables we need here are `sex` and `cs02` and the values we want to filter for are 1 (male), and 1 (never), respectively.
```
...
## Idee - Internet use instead of age
```{block, box.title = "3", box.body = list(fill = "white"), box.icon = "fa-star"}
Using the same function from `dplyr`, create another subset of cases called `Allbus_social_media_news` that only includes respondents that stated that they use social media between *three to five* days per week as a means of news source.
```

```{block, opts.label = "clues"}
The variable we need for this is called `lm35` and the values of that variable we are looking for are 5 to 7 *(?)*. You can use the helper function `between()` here (remember that the values you provide to this function are inclusive).
```

```{r third-exercise, solution = TRUE}
Allbus_social_media_news <- Allbus %>% 
  filter(between(lm35, 5, 7))
```
...
```{block, opts.label = "clues"}
You need the `base R` function `order()` here. You can check your result using `head()`. To limit the amount of output, you can subset columns using [ ] within the `head()` command (general life satisfaction is the *XXXXXXth* variable in the data set, so you could, e.g., subset columns 6:*XX*).
```
...
*Why are we reloading the data set?* 
...
```{r read-data, message = F}
library(tidyverse)
gp_covid <- read_csv2("./data/XXXXXXXX.csv")
```
...
### TODO: check missing variable types
```{block, box.title = "3", box.body = list(fill = "white"), box.icon = "fa-star"}
...
```{block, box.title = "4", box.body = list(fill = "white"), box.icon = "fa-star"}
After recoding a set of values as `NA` for one variable, let's now do the same for the whole `Allbus` data frame. This time, however, we do not want to recode 98 as `NA` as it is a valid value for the `id` variable.
```
...
Allbus <- Allbus %>% 
  mutate(age_3cat = case_when(
    between(yborn, 1, 40) ~ "up to 40 years",
    between(yborn, 41, 60) ~ "41 to 60 years",
    yborn > 60 ~ "older than 60 years"
    ))
```
...
For this exercise, we will use the same subset of the *German General Social Survey - ALLBUS 2021* data as in the lecture. If you have stored that data set as an `.rds` file as shown in the slides, you can simply load it with the following command:
...
###TODO: new variables with interesting summ stats
```{block, box.title = "1", box.body = list(fill = "white"), box.icon = "fa-star"}
...
```{block, box.title = "2", box.body = list(fill = "white"), box.icon = "fa-star"}
Using a function from the `psych` package, print the summary statistics for all variables that assess how much people trust specific people or institutions. The summary statistics should include IQR but no measures of skew (and kurtosis).
```
...
```{block, box.title = "3", box.body = list(fill = "white"), box.icon = "fa-star"}
Use a function from the `summarytools` package to get summary statistics for the following variables in your data set: `pa01` (left-right self-placement), `sum_measures` *?*, `mean_trust` *?*. Unlike in the lecture, however, we now want all stats (not just the "common" ones).
```
...
```{block, box.title = "1", box.body = list(fill = "white"), box.icon = "fa-star"}
As a first exercise, use `base R` to create a cross tabulation for the variables `age_cat` (rows) and `choice_of_party` (columns) showing row percentages.
```
...
## Idea:*six* distinct strings right?
...
## Idea: call the variables 'first' and 'second' as stated in the description.
```{r sixth.exercise, solution = TRUE}
names(cool_data_frame) <- c("first", "second")
cool_data_frame
```
...
```{r load_gp, solution = TRUE}
library(haven)
Allbus <- read_dta("H:/RWorkshop_SuSch/Data/ZA5270_v2-0-0.dta")
```
