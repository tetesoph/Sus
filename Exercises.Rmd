...
Allbus_subset <- Allbus_subset %>% 
    mutate(sex = recode_factor(sex,
           ...
           educ = recode_factor(educ,
                                   ...
                                   `6` = "Abitur",
                                   .ordered = TRUE),
           choice_of_party = recode_factor(pv01,
                                           `1`= "CDU/CSU",
                                           `2`= "SPD",
                                           `3` = "AfD",
                                           `4` = "FDP",
...
Allbus <- 
  ...
  sjlabelled::set_na(na = c(-1:-99))
```
### AN: Changed .sav file to.csv file
...
plot(
  jitter(Allbus$pa01, 2), 
  jitter(Allbus$fr03b, 2)
)
```
...
tab_1 <- table(Allbus$fr07)
tab_2 <- table(Allbus$fr08)
tab_3 <- table(Allbus$fr03b)
tab_4 <- table(Allbus$fr04b)
par(mfrow = c(2, 2))
barplot(tab_1)
barplot(tab_2)
barplot(tab_3)
...
boxplot(Allbus$fr07)
boxplot(Allbus$fr08)
boxplot(Allbus$fr03b)
boxplot(Allbus$fr04b)
```
...
gapminder_ggplot_input <-
  readr::read_csv("gdppercapita_us_inflation_adjusted.csv") %>%
...
```{block, box.title = "1", box.body = list(fill = "white"), box.icon = "fa-star"}
Let's start with a very simple analysis: Compute a t-test to compare the means of the two `sex` groups in the data for the variable `sum_measures`.
```
...
```{r first-exercise, solution = TRUE}
t.test(sum_measures ~ dh01, data = Allbus_subset)
```
