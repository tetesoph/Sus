...
### Problem: es gibt nicht viele binär codierte variablen...entweder so lassen oder zweimal den mean ausrechnen
...
Allbus_subset %>% 
  ...
  drop_na() %>% 
  group_by(agec) %>% 
  summarize(risk_self_mean = mean(lm19),
...
```{r}
saveRDS(corona_survey, "Allbus_subset.rds")
```
...
```{r load}
Allbus_subset <- readRDS("Allbus_subset.rds")
```
...
```{r first-exercise, solution = TRUE}
table(Allbus_subset$education_cat, useNA = "always")
```
